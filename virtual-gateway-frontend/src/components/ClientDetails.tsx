...
export default function ClientDetails() {
  ...
  const sendMessageType = async (type: "NEW_DEVICE" | "DEVICE_REMOVED" | "DEVICE_DELETED" | "START_REPORTING_METERS" | "DELETE_METERS" | "ENABLE_AUTO_CLOSE" | "DISABLE_AUTO_CLOSE" | "EVENT_NOTIFICATION_REQUEST" | "GLO_EVENT_NOTIFICATION_REQUEST", meterId: number | null) => {
    if (!id) return;
    try {
      setIsSending(true);
    ...
    <div className="bg-white dark:bg-gray-800 dark:text-gray-200 p-6 rounded shadow">
      ...
      <div className="flex flex-row items-center justify-between mb-4">
        ...
        <div className="flex flex-row gap-2">
          <button disabled={!isConnected} onClick={() => sendMessageType("NEW_DEVICE", null)} className="btn btn-sm btn-circle border-none shadow-none bg-cyan-500 hover:bg-cyan-600 text-white">
            <i className="fa-solid fa-plus"></i>
          </button>
          <button disabled={!isConnected} onClick={closeClientSocket} className="btn btn-sm btn-circle border-none shadow-none bg-red-600 hover:bg-red-700 text-white">
          ...
          <div className="stat-value text-lg">
            <div className="tooltip tooltip-right" data-tip={isConnected ? "Conectado" : "Desconectado"}>
              <div className="inline-grid *:[grid-area:1/1]">
                <div className={`status status-xl ${isConnected ? 'status-success' : 'status-error'} animate-ping`}></div>
                <div className={`status status-xl ${isConnected ? 'status-success' : 'status-error'}`}></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div key={index} className="collapse collapse-plus bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-800 shadow-xl mb-2">
            ...
            <div className="collapse-content text-sm">
              ...
              <div className="flex flex-row gap-2 my-4">
                ...
                <button disabled={isSending || !isConnected || !meter.connected}
                  className={`btn border-none shadow-none btn-circle ${isConnected ? "bg-amber-300 hover:bg-amber-400" : "bg-gray-400"} text-white transition`}
                  onClick={() => sendMessageType("EVENT_NOTIFICATION_REQUEST", meter.deviceId)}>
                  <i className="fa-solid fa-alarm-exclamation"></i>
                </button>

                <button disabled={isSending || !isConnected || !meter.connected}
                  className={`btn border-none shadow-none btn-circle ${isConnected ? "bg-amber-300 hover:bg-amber-400" : "bg-gray-400"} text-white transition`}
                  onClick={() => sendMessageType("GLO_EVENT_NOTIFICATION_REQUEST", meter.deviceId)}>
                  <i className="fa-regular fa-sensor-triangle-exclamation"></i>
                </button>

              </div>

              <div className="overflow-y-auto rounded-box bg-base-100 dark:bg-gray-500">
